...
- 后端：`python -m pytest -q`，443 项通过，0 项失败；仅有 Starlette 关于测试客户端依赖的弃用警告。
- 管理端：TypeScript 类型检查通过。
- 管理端：生产构建通过。
- 专项覆盖：Agent 工具循环、真实卡片、未核实价格 / 库存 / 发送状态拦截、单问题自然表达、新好友开场结构与专用队列、资料资产、订单状态、输入过滤、微信队列、每日触达幂等 / 计数 / 重试 / 租约恢复和人工接管。
