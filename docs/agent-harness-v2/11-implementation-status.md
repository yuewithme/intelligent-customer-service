...
- 后端：`python -m pytest -q`，446 项通过，0 项失败；仅有 Starlette 关于测试客户端依赖的弃用警告。
- 本次未修改管理端；最近一次 2026-08-05 验证中，TypeScript 类型检查和生产构建均通过。
- 专项覆盖：Agent 工具循环、关系经营优先与成熟购买直达、真实卡片、未核实价格 / 库存 / 发送状态拦截、客户收益导向的单问题挖需、语义消息单元保留、新好友开场结构与专用队列、资料资产、订单状态、输入过滤、微信队列、每日触达幂等 / 计数 / 重试 / 租约恢复和人工接管。
