...
旧的 `INTENT_*`、`REPLY_SHADOW_*`、`TALK_SCRIPT_LLM_*`、固定开场内容和 Qdrant 意图 / 话术集合配置已从配置契约及示例环境文件移除。旧部署环境中即使暂时保留这些键，也会因 Settings 的额外字段忽略策略而不参与运行。

## 验证结果

2026-08-05 本地最终验证：

- 后端：`python -m pytest -q`，439 项通过，0 项失败；仅有 Starlette 关于测试客户端依赖的弃用警告。
- 管理端：TypeScript 类型检查通过。
- 管理端：生产构建通过。
- 专项覆盖：Agent 工具循环、真实卡片、未核实价格 / 库存 / 发送状态拦截、资料资产、订单状态、输入过滤、微信队列、每日触达幂等 / 计数 / 重试 / 租约恢复和人工接管。
