...
默认销售思路已经调整为“先收集足以匹配的客户事实，再主动推荐”：新客户先理解来意，围绕当前话题每轮只补一个最有价值的信息点，同时提供真实帮助并自然塑造团队价值。新好友身份、普通兰花话题或索要资料本身不会触发商品或资料发送；当客户需求和关键条件已经足以解释适配理由时，即使客户没有说固定购买口令，Agent 也会主动查询并推荐。资料只有在能说明匹配、释放目的和后续动作时才发送，发送后继续挖需或转入产品推荐。这是核心提示和渐进披露经验，不是新增阶段机、关键词路由或固定字段表单。
...
- 后端：`python -m pytest -q`，448 项通过，0 项失败；仅有 Starlette 关于测试客户端依赖的弃用警告。
- 本次未修改管理端；最近一次 2026-08-05 验证中，TypeScript 类型检查和生产构建均通过。
- 专项覆盖：Agent 工具循环、前期逐轮信息收集、信息不足时不释放资料、事实充分后的资料释放与主动推品、成熟购买直达、真实卡片、未核实价格 / 库存 / 发送状态拦截、客户收益导向的单问题挖需、语义消息单元保留、新好友开场结构与专用队列、订单状态、输入过滤、微信队列、每日触达幂等 / 计数 / 重试 / 租约恢复和人工接管。
