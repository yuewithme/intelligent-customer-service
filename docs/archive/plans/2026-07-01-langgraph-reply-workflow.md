...
```text
POST /api/v1/chat
  -> app.domains.conversations.api.chat.chat
  -> app.domains.conversations.services.chat_orchestrator.handle_chat
...
def test_reply_workflow_graph_imports():
    from app.domains.decisioning.services.reply_workflow_graph import build_reply_workflow_graph
...
from app.domains.decisioning.schemas.reply import FinalReply
from app.domains.sales.talk_script.service import match_talk_script
```
...
from app.domains.decisioning.services.reply_builder import build_rag_reply
from app.domains.conversations.services.chat_orchestrator import _is_rag_no_answer, build_handoff_reply
```
...
if get_settings().reply_graph_enabled:
    from app.domains.decisioning.services.reply_workflow_graph import build_reply_with_graph
...
Monkeypatch `app.domains.decisioning.services.reply_workflow_graph.build_reply_with_graph` to return a known `FinalReply`; call `handle_chat` with a safe message and assert response contains that reply.
