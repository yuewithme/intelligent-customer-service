...
app/integrations/youzan/services/youzan_product_service.py
    商品查询和字段标准化

app/integrations/youzan/services/youzan_order_service.py
    手机号 → yz_uid → 订单列表

app/domains/catalog/services/commerce_query_service.py
    对话状态、手机号提取、商品/订单工具编排

app/domains/decisioning/services/intent_service.py
    product_query / order_query 识别及手机号续问

app/domains/decisioning/services/business_reply_renderer.py
    确定性商品/订单回复和小程序卡片生成

app/integrations/eyun/services/eyun_callback_service.py
    /sendApplets 调用

app/integrations/eyun/services/message_risk_control_service.py
    文本和小程序卡片出站队列
```
...
- 手机号查询成功后，按业务要求完整持久化手机号，同时保存脱敏值和有赞身份标识；同一微信客户后续查单无需重复发送手机号。完整手机号不得出现在 AI 返回、运行诊断日志和工具审计参数中。
...
- MCP 新增商品搜索、商品详情、库存、客户识别、客户订单和受归属校验保护的订单详情工具；所有调用写入脱敏审计记录并返回 `trace_id`。
- 生产数据由 `/srv/intelligent-customer-service/data/` bind mount 到容器 `/app/data`；仓库内不保存运行数据。
