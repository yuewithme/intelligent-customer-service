...
|---|---|---|
| Identity | Map channel identities to one subject | `app/domains/customers/services/memory_identity_service.py` |
| Event store | Idempotent append and source lookup | `app/domains/customers/services/memory_event_service.py` |
| Job queue | Durable claim, retry, and dead-letter state | `app/domains/customers/services/memory_job_service.py` |
| Extraction | Produce candidate memory operations | `app/domains/customers/services/memory_extraction_service.py` |
| Validation | Enforce evidence and source policy | `app/domains/customers/services/memory_validation_service.py` |
| Consolidation | Apply versioned operations transactionally | `app/domains/customers/services/memory_consolidation_service.py` |
| Projection | Rebuild the compatible user profile | `app/domains/customers/services/memory_projection_service.py` |
| Retrieval | Scope, recall, rerank, and evidence expansion | `app/domains/customers/services/memory_retrieval_service.py` |
| Vector index | Maintain rebuildable episode vectors | `app/domains/customers/services/memory_vector_service.py` |
| Operations | Shadow metrics, gates, reviewed procedures | `app/domains/customers/services/memory_procedure_service.py` |
