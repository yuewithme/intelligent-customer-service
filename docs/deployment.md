...
- 首次初始化账号表时创建 `admin`，密码使用现有 `ADMIN_GATE_PASSWORD`；未配置时使用 `API_KEY`，拒绝默认值 `change_me`。如配置 `ADMIN_GATE_TEST_PASSWORD`，同时初始化 `test` 账号。
- 管理员从“系统 → 账号与权限”创建账号、生成或重置密码、启停账号，以及分配页面和员工微信范围。创建和重置返回的密码只展示一次；数据库仅保存加盐 scrypt 哈希。
