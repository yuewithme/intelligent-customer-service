...
后台使用数据库账号密码登录，无注册入口。升级后旧门禁 Cookie 失效，需要重新登录。

- 首次初始化账号表时创建 `ADMIN_GATE_USERNAME` 指定的管理员（默认 `wohukeji`），密码使用 `ADMIN_GATE_PASSWORD`；未配置时使用 `API_KEY`，拒绝默认值 `change_me`。如配置 `ADMIN_GATE_TEST_PASSWORD`，同时初始化 `test` 账号。已存在的管理员由后台编辑账号，不因重启覆盖账密。
- 管理员从“系统 → 账号与权限”新增、编辑、删除账号，设置登录名和至少 6 位密码、启停账号，以及分配页面和员工微信范围。新增时密码留空自动生成；编辑时密码留空保留原密码。数据库仅保存加盐 scrypt 哈希。
- 修改登录名或密码、删除账号会立即撤销其全部登录会话。管理员不能删除、停用自己或移除自己的管理员身份；删除账号保留历史业务记录。
- 初始化只执行一次；后续修改环境变量不会覆盖已有账号密码。密码重置、账号停用和退出登录会撤销数据库会话，会话有效期为 12 小时。
- 页面权限和员工微信范围在每次 API 请求时核验。员工未选择微信时默认全部微信，选择后仅允许所选微信；页面与角色限制仍生效。跨微信共用的客户资料因历史上按客户 ID 聚合，只有员工覆盖该客户全部关联微信时才开放，防止泄露未授权微信的数据。
- 账号、会话和登录限流表保存在 `DATABASE_URL` 指向的数据库（生产 `/app/data/rag.db`），随现有数据备份一起保留。发布不需要手动执行 SQL；服务启动时自动建表。
- 浏览器 Nginx 不再注入服务端 API 密钥；服务端集成仍可使用自己的 Bearer 凭据。`API_AUTH_ENABLED` 在生产必须保持 `true`。
