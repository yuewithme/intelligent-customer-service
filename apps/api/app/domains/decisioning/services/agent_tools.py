...
CAPABILITIES = (
    ...
    CapabilitySpec(
        ...
        "适用于已验证已购客户的养护或品种问题已经得到有效解决，客户用‘好的、谢谢、明白了、知道了’等表示认可并自然结束的节点。目标不是本轮立即卖货，而是当轮完整留下三个长期关系结果，而且三项缺一不可：第一条消息告诉客户后续仍可带着照片和具体问题来咨询；第二条消息说明每周会上新多款铭品供鉴赏、有会员专属折扣活动、以后有实拍图和优惠会主动分享；第三条消息轻量询问一个尚未知的花色、瓣型、香味或品种鉴赏偏好，并说明是为了以后针对性分享、买不买都没关系。三个动作必须拆成三条独立 text 消息，不能把它们合并在一条，也不能因为客户正在自然结束就省略其中任何一项。客户追问具体上新日、数量、品种、折扣、金额、条件或有效期时不编造；简短说明每期安排不同，有合适内容会按其喜好分享，顺势转问一个尚未知的鉴赏偏好。这叫长期偏好采集，不等于客户已有购买意向，不触发 product.search、商品推荐或卡片。客户回答后，用 memory.record 保存有原话证据的稳定偏好，后续分享时使用，不能反复再问。若当前问题未解决、仍需观察变化、存在投诉售后或权限待办、客户明确拒绝或表现不耐烦、上一问题尚未回答，则不触发三段收口，优先完成服务或尊重拒绝。没有可信会员身份时称已购兰友，不擅自称专属会员。",
        ("service",),
    ),
    CapabilitySpec(
