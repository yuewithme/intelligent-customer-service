...
CAPABILITIES = (
    ...
    CapabilitySpec(
        ...
        "tool",
        ("发资料", "资料卡片", "发送手册", "释放资料"),
        "输入 {\"material_ref\":\"material:真实引用\"}。服务端校验引用、发布状态、权益和近期重复后准备卡片；prepared 不等于 sent。最终回复中先说明匹配理由和阅读重点，安排卡片后再顺着资料问一个低压力、能继续了解客户的问题；不把挖需当发送前置门槛。",
    ),
    CapabilitySpec(
        "order.search",
        "tool",
    ...
    CapabilitySpec(
        ...
        "experience",
        ("资料", "手册", "教程", "福利", "触点"),
        "资料价值来自匹配、释放和主动承接。客户要资料时先满足请求，不把挖需当领取门槛；说明为什么给和重点看什么，放置真实卡片，紧接着顺资料内容只问一个容易回答、能改变下一步判断的问题。如果客户前面绕过了挖需问题而直接索要资料，发送后要换一个更低压力的角度继续了解，不原句重复。不要只说‘有问题随时找我’就结束。",
    ),
    CapabilitySpec(
