...
CAPABILITIES = (
    ...
    CapabilitySpec(
        ...
        "客户已表达选购或购买意向时可直接使用；客户未主动要商品时，只要已知需求、目标和关键适配条件足以说清推荐理由，也可主动查询。如果一个未知仍会明显改变选品，再自然了解这一项；新好友身份、普通兴趣和空泛养兰话题本身不足以调用。输入 {\"query\":\"商品名或已明确的匹配需求\",\"limit\":3}；搜索不会发送卡片。",
    ),
    CapabilitySpec(
        "product.get",
        "tool",
