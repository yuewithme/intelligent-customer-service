...
CAPABILITIES = (
    ...
    CapabilitySpec(
        ...
        "适用于已验证已购客户的养护或品种问题已经得到有效解决、对话准备自然收口的节点。目标不是本轮立即卖货，而是依次留下三个长期关系结果：让客户知道后续仍可带着照片和具体问题来咨询；说明会员内容与福利吸引点；在气氛自然且客户没有收口、拒绝或不耐烦时，轻量了解一个尚未知的长期鉴赏偏好，例如花色、瓣型、香味或品种方向。三项是判断顺序，不是要求同一轮连续发送的三段话。单轮最多组合两个相邻动作：可用‘持续咨询＋会员钩子’完成当轮，等客户回应后再问偏好；也可在前文已经讲清持续咨询时，用‘会员钩子＋一个偏好问题’推进。绝不能在同一轮同时出现持续咨询、会员钩子和偏好提问三项。默认一到两条短消息，最多问一个容易回答的问题；前一项已在近期讲清就跳到尚未完成且仍自然的一项。会员吸引点可以直接说每周会上新多款铭品供鉴赏、有会员专属折扣活动、以后有实拍图和优惠会主动分享。客户追问具体上新日、数量、品种、折扣、金额、条件或有效期时不编造；简短说明每期安排不同，有合适内容会按其喜好分享，顺势转问一个尚未知的鉴赏偏好。询问偏好时要说明是为了以后更有针对性地分享鉴赏，买不买都没关系；这叫长期偏好采集，不等于客户已有购买意向，不触发 product.search、商品推荐或卡片。客户回答后，用 memory.record 保存有原话证据的稳定偏好，后续分享时使用，不能反复再问。若当前问题未解决、仍需观察变化、存在投诉售后或权限待办、客户明确结束/拒绝、上一问题尚未回答，或画像与近期对话已有该偏好，就停止推进；优先完成服务或自然收口。没有可信会员身份时称已购兰友，不擅自称专属会员。",
        ("service",),
    ),
    CapabilitySpec(
