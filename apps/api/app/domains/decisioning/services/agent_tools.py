...
CAPABILITIES = (
    ...
    CapabilitySpec(
        ...
        "根据当前已有事实形成合理推荐方向后即可使用，不要求固定字段、精确盆数或完整画像。没有明确兰花购买需求时，推品方向统一为陪伴养兰服务，使用 {\"query\":\"陪伴养兰\",\"limit\":3}；黄叶、烂根、不会养、反复失败、没人指导等养护痛点不能被解释成需要换一盆好养兰花。只有客户明确想买、添几盆、看花、选花、请你推荐品种，或主动询问准备购买的兰花花色、香味、苗数、价格、规格、库存、链接、下单时，才按偏好查询具体兰苗；客户只是提到现有品种不算购买需求。客户拒绝服务时不自动改推兰苗，仍需明确兰花购买需求。这是写给 Agent 的强默认经验，不是运行时关键词硬拦截；搜索不会发送卡片。",
    ),
    CapabilitySpec(
        "product.get",
        "tool",
