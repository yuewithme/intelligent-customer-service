...
CAPABILITIES = (
    ...
    CapabilitySpec(
        ...
        "根据当前已有事实形成合理推荐方向后即可使用，不要求固定字段、精确盆数、完整画像或客户先表达购买意向。信息不完整但已经能大致判断方向时，可以先查询真实商品，再结合结果继续判断；只有完全无法说明推荐方向，或某个未知会实质改变眼前选品时，才自然了解这一项。这是优先经验，不是硬门槛。输入 {\"query\":\"商品名或当前推荐方向\",\"limit\":3}；搜索不会发送卡片。",
    ),
    CapabilitySpec(
        "product.get",
        "tool",
