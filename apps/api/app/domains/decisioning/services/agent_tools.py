...
CAPABILITIES = (
    ...
    CapabilitySpec(
        "brand.service_facts",
        "tool",
        ("萧岚苑", "服务差异", "同行", "手把手", "一对一", "单品养护", "买后服务"),
        "输入 {}。返回可对外表达的已核实买后服务事实：客户在萧岚苑结缘兰花后有对应品种的单品养护教程；教程看完或实操中有不懂，可由养兰师傅结合品种、地区和环境一对一指导。对比只能说市面上有些商家卖完后缺少持续养护承接，不能贬低所有同行或承诺一定养活。",
    ),
    CapabilitySpec(
        "knowledge.search",
        "tool",
    ...
    CapabilitySpec(
        ...
        "根据当前已有事实形成合理推荐方向后即可使用，不要求固定字段、精确盆数或完整画像。先做柔性方向加权：客户只有黄叶、烂根、不会养、反复失败、没人指导等养护痛点而没有兰花购买意向时，通常优先查询陪伴养兰服务，使用 {\"query\":\"陪伴养兰\",\"limit\":3}；不要仅把养护痛点解释成需要换一盆好养兰花。客户出现想买、添花、看花、选品，或主动问品种、花色、香味、苗数、价格、规格、库存、链接、下单时，再提高具体兰苗权重并按偏好查询。如果两类信号并存，可灵活组合。这是默认权重和销售经验，不是运行时硬门槛；搜索不会发送卡片。",
    ),
    CapabilitySpec(
        "product.get",
        "tool",
...
async def _brand_service_facts(*, call_id, arguments, context) -> AgentToolResult:
    ...
    return _result(
        ...
        comparison_boundary="可以说市面上有些商家卖完苗后缺少持续养护承接；不能说所有同行都只卖不教、苗不好或不负责。",
        delivery=[
            {
                "name": "对应品种的单品养护教程",
        ...
        claim_boundaries=[
            "不承诺一定养活、一定开花或任何疗效。",
            "具体商品、价格、库存、活动和额外会员权益仍需通过对应工具核实。",
        ],
    )
