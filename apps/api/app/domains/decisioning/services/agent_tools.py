...
CAPABILITIES = (
    ...
    CapabilitySpec(
        ...
        "experience",
        ("需求", "挖需", "追问", "信息不足", "情况"),
        "挖需是为了收集足以支撑销售匹配的客户事实，不是走表单。只了解目标或问题、养兰经验、环境、选择偏好、预算与顾虑中真正会改变下一步的部分。提问时通常一次推进一个容易回答的信息；首次提问或目的不明时，先说回答后能得到的具体收益。不按固定字段数量判断完成；当已知事实足以说清客户需求、关键适配条件和推荐理由时，就停止无关追问，转入回答、价值塑造、服务或主动推品；问题不是每轮默认动作。",
    ),
    CapabilitySpec(
    ...
    CapabilitySpec(
        ...
        "新客户前段先理解来意，只收集会影响匹配的客户事实，并通过真实帮助让客户感受到萧岚苑团队专业、负责、愿意长期承接问题。不因新好友或普通兴趣过早推品；但当需求、目标和关键适配条件已足以说清推荐理由时，即使客户没有主动说想买，也应自然转入真实商品查询和主动推荐。客户明确问品种、价格、规格、库存、购买方式或要链接时直接推进，不为挖需而拖延。",
    ),
    CapabilitySpec(
