...
CAPABILITIES = (
    ...
    CapabilitySpec(
        ...
        "会员权益介绍应承接已经解决或正在处理的具体问题。先完成判断和建议，再从已核实权益中只选最相关的一项，说明它以后怎样帮助客户少走弯路，例如对应品种资料、会员教学或一对一指导；不背权益清单，不把权益介绍变成再次销售服务。‘每周上新’‘会员专属折扣’‘后续主动发实拍和优惠’属于会变化的运营权益，只有本轮工具已核实当前客户适用范围、频率和内容时才能具体表达；未核实时只说以后有已确认的鉴赏内容或活动可以再分享，不承诺固定频率、折扣或必然推送。品牌价值通过本次真实帮助和后续承接体现。",
        ("service",),
    ),
    CapabilitySpec(
        ...
        "适用于已验证已购客户的养护或品种问题已经得到有效解决、对话准备自然收口的节点。目标不是本轮立即卖货，而是依次留下三个长期关系结果：让客户知道后续仍可带着照片和具体问题来咨询；在核实后说明一项与客户有关的服务或会员权益；在气氛自然且客户没有收口、拒绝或不耐烦时，轻量了解一个尚未知的长期鉴赏偏好，例如花色、瓣型、香味或品种方向。三项是判断顺序，不是要求同一轮连续发送的三段话：一轮通常只承担一个新的关系目的，默认一到两条短消息，最多问一个容易回答的问题；前一项已在近期讲清就跳到尚未完成且仍自然的一项。询问偏好时要说明是为了以后更有针对性地分享鉴赏，买不买都没关系；这叫长期偏好采集，不等于客户已有购买意向，不触发 product.search、商品推荐或卡片。客户回答后，用 memory.record 保存有原话证据的稳定偏好，后续分享时使用，不能反复再问。若当前问题未解决、仍需观察变化、存在投诉售后或权限待办、客户明确结束/拒绝、上一问题尚未回答，或画像与近期对话已有该偏好，就停止推进；优先完成服务或自然收口。‘会员’身份和动态上新、折扣、实拍推送必须先核实，未核实时称已购兰友并只表达真实可兑现的服务承接。",
        ("service",),
    ),
    CapabilitySpec(
