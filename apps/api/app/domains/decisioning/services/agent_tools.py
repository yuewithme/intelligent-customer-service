...
CAPABILITIES = (
    ...
    CapabilitySpec(
        ...
        "输入 {}。返回可对外表达的已核实品牌规模、服务效果、风险数据、统一补发寄养赠品权益、选品事实，以及包含教程和一对一指导的已核实买后服务事实。数据可用于案例证明、同行对比、痛点放大和成交推进，但历史比例不等于对单个客户保证相同结果。网上和花鸟市场来源中，不对版、温室或激素苗、适应性和售后不足是常见且占比较大的风险方向；可以说客户的情况‘可能是、常见于、大概率与这些方向有关’，不能只凭来源断言他手上的兰花一定如此。",
    ),
    CapabilitySpec(
        "knowledge.search",
        "tool",
