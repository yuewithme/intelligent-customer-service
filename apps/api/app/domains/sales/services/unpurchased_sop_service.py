...
async def sync_eyun_contacts(*, friend_ids: list[str] | None = None) -> dict[str, Any]:
    ...
    if friend_ids is None:
        with _get_session() as session:
            ...
            session.commit()
        if baseline_pending:
            from app.integrations.eyun.services.eyun_contact_service import initialize_eyun_contacts
...
async def _refresh_new_contact_details(wc_ids: list[str], w_id: str) -> None:
    if not wc_ids or not w_id:
        return
    from app.integrations.eyun.services.eyun_contact_service import get_eyun_contact_snapshots
