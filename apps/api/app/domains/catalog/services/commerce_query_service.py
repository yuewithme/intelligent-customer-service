...
MEMBERSHIP_CARE_PAIN_SALES_SCRIPT = (
    "{profile_lead}{service_need}不能只处理眼前的表现，还要把相关的浇水、通风、光照和植料状态一起理顺，"
    "否则很容易刚解决一个环节又在另一个环节出问题。\n\n"
    "我们萧岚苑服务过很多有类似情况的兰友，会先帮您把问题和养护思路理清楚，"
    "后面再结合家里的实际环境陪着调整，尽量避免同样的问题反复出现。"
)
MEMBERSHIP_CARE_PAIN_ROOT_ROT_SCRIPT = (
    "{profile_lead}先把发黑、发软或空掉的根剪干净，伤口消毒晾干后，再用透气些的植料重新上盆；"
    "缓苗期间先别急着浇大水。{service_need}处理完以后，如果浇水和通风节奏没跟着调整，后面还容易反复。\n\n"
    "我们萧岚苑服务过很多有类似情况的兰友，除了把当下的处理方法讲清楚，"
    "也会结合家里的环境陪着调整，把日常养护慢慢理顺。"
)
MEMBERSHIP_CARE_PAIN_BLACK_SPOT_SCRIPT = (
    "{profile_lead}先把病斑明显的叶片剪掉并和其他兰花隔开，放在通风散光处，浇水时别让叶心长期积水。"
    "{service_need}处理只是第一步，后面的湿度和换季预防没有调好，病斑仍然可能反复。\n\n"
    "我们萧岚苑更重视陪兰友把预防和日常养护一起理顺，遇到变化时也能结合实际情况及时调整。"
)
MEMBERSHIP_CARE_PAIN_YELLOW_TIP_SCRIPT = (
    "{profile_lead}先别急着补肥，可以看看是老叶均匀退黄，还是叶尖先干焦，再结合盆内干湿和根系状态判断。"
    "{service_need}只是表面表现，背后还要把水肥、光照和空气湿度逐项排查。\n\n"
    "我们萧岚苑服务过很多有类似情况的兰友，会陪着把这些基础环节逐步理清，避免只处理表面后又反复。"
)
MEMBERSHIP_CARE_PAIN_NO_BLOOM_SCRIPT = (
    "{profile_lead}先确认这个品种是否需要春化，再看苗情、光照和肥力是否达到开花条件。"
    "{service_need}不一定只是晒太阳少，需要结合品种和环境一起调整。\n\n"
