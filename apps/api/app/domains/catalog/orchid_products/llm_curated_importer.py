    ...
    with closing(load_workbook(path, read_only=True, data_only=True)) as workbook:
        ...
        for sheet_name in workbook.sheetnames:
            ...
            if sheet_name in VARIETY_SHEETS:
                ...
                _curate_variety_rows(payload, sheet_name, read_records(sheet, ["品种名"]))
            elif sheet_name == COMMON_KNOWLEDGE_SHEET:
                _curate_common_rows(payload, read_records(sheet, ["分类", "共享话术"]))
            elif sheet_name == SKU_SHEET:
                _curate_sku_rows(payload, read_records(sheet, ["品种", "品名"]))
            elif sheet_name == SALES_COPY_SHEET:
                _curate_sales_rows(payload, read_records(sheet, ["填写人", "品种名"]))
            elif sheet_name == HOT_BREAKDOWN_SHEET:
                _curate_hot_rows(payload, read_records(sheet, ["品种名", "品类"]))
...
def _curate_variety_rows(
    ...
    for record in records:
        ...
        if not variety_name:
            continue
        alias = cell_text(record.get("别名")) or None
        raw_basic_info = cell_text(record.get("产品基础信息")) or None
...
def _curate_sku_rows(
    ...
    previous_name = ""
    for record in records:
        variety_name = cell_text(record.get("品名")) or previous_name
        if not variety_name:
            continue
...
def _curate_sales_rows(
    ...
    for record in records:
        ...
        for source_field, chunk_type in chunk_fields:
            content = cell_text(record.get(source_field))
            if content:
                payload.knowledge_chunks.append(
                    build_chunk(
                        "orchid_sales_copy",
