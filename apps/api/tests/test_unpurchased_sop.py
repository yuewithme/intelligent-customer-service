...
from app.domains.customers.services.user_profile_service import get_profile_bundle, patch_user_profile
...
def test_step_time_range_generates_stable_random_due_time(monkeypatch, tmp_path):
    ...
    with get_sop_session() as session:
        from app.infrastructure.database.models import UnpurchasedSopStepModel
...
async def test_contact_identity_stores_remark_and_wechat_alias(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "app.integrations.eyun.services.eyun_contact_service.get_eyun_contact_snapshots",
        fake_snapshots,
    )
...
async def test_risk_queue_cancels_dependent_message_after_previous_failure(monkeypatch, tmp_path):
    _settings(monkeypatch, tmp_path)
    from app.infrastructure.database.models import ConversationMessageModel, EyunOutboundMessageModel
