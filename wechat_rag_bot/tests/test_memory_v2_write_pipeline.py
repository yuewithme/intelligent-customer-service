...
from app.services.memory_event_service import append_memory_event, list_memory_events
...
async def test_lifespan_starts_memory_worker_only_when_write_flag_enabled(monkeypatch):
    ...
    monkeypatch.setattr(
        main_module, "eyun_risk_control_worker", lambda stop: worker(stop, "eyun")
    )
    monkeypatch.setattr(
        main_module, "unpurchased_sop_worker", lambda stop: worker(stop, "sop")
    )
    monkeypatch.setattr(
        main_module, "youzan_product_sync_worker", lambda stop: worker(stop, "youzan")
    )
    monkeypatch.setattr(
        main_module, "memory_worker", lambda stop: worker(stop, "memory")
    )
    monkeypatch.setattr(
        main_module, "run_sales_mcp_session_manager", fake_mcp_manager
    )
    app = type("App", (), {"state": type("State", (), {})()})()

    async with main_module.lifespan(app):
        await asyncio.sleep(0)
        assert set(started) == {"eyun", "sop", "youzan", "memory"}
