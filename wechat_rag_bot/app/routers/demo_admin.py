...
from app.utils.auth import require_gate_access


router = APIRouter(
    prefix="/api/v1/demo-admin/conversations",
    tags=["demo-admin"],
    dependencies=[Depends(require_gate_access)],
)
...
async def conversation_detail(conversation_id: str) -> APIResponse:
    await _ensure_demo_conversation(conversation_id)
    return APIResponse(
        code=0, message="success", data=await get_conversation_detail(conversation_id)
    )
...
async def mark_read(conversation_id: str) -> APIResponse:
    await _ensure_demo_conversation(conversation_id)
    return APIResponse(
        code=0, message="success", data=await mark_conversation_read(conversation_id)
    )
